...
public class EditUserHandler : ICommandHandler<EditUserDataResponse, EditUserCommand> {
  ...
  public async Task<Result<EditUserDataResponse>> Handle (
    ...
    // 2. Обновляем UserName (логин)
    if (!string.IsNullOrWhiteSpace (command.Request.Fullname) && !string.Equals (user.Fullname, command.Request.Fullname
          , StringComparison.OrdinalIgnoreCase)) {
      var setNameResult = await _userManager.SetUserNameAsync (user, command.Request.Fullname);
