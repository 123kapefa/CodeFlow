...
public sealed class ReputationRepository : IReputationRepository {
  ...
  public async Task<IReadOnlyList<UserReputationChanged>> ApplyAcceptedAnswerAsync (
    ...
    if (oldOwnerUserId is not null) {
      var eff = await GetOrCreateEffect ((Guid)oldOwnerUserId, parentId, newAnswerId, ReputationSourceType.Answer,
        "AcceptedAnswer", sourceService, ct);
      if (oldDelta != 0) {
        var applied = eff.Apply (oldDelta, eventId);
        if (applied) {
          _context.ReputationEntries.Add (ReputationEntry.Create ((Guid)oldOwnerUserId, parentId, newAnswerId,
            ReputationSourceType.Answer, "AcceptedAnswer", reason, oldDelta, occurredAt, eventId, sourceService));
