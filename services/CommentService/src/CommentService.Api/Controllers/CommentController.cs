...
public class CommentController : ControllerBase {
  ...
  [SwaggerOperation (
    ...
    Description = "Возвращает IEnumerable<CommentDTO> .", 
    OperationId = "Comment_Get")]
  public async Task<Result<IEnumerable<CommentDTO>>> GetQuestionCommentsAsync (
    ...
    await handler.Handle (
      new GetCommentsCommand (TypeTarget.Question, targetId), 
      new CancellationToken (false));

  [HttpPost ("get-comments-by-answers")]
  [SwaggerOperation (
    Summary = "Получить комментарии для списка ответов.",
    Description = "Возвращает список комментариев (CommentDTO) по списку идентификаторов ответов.",
    OperationId = "Comment_GetByMultipleAnswers")]
  public async Task<Result<IEnumerable<CommentDTO>>> GetCommentsByAnswerIdsAsync (
    [FromBody] IEnumerable<Guid> answerIds,
    [FromServices] ICommandHandler <IEnumerable<CommentDTO>, GetCommentsByAnswerIdsCommand> handler) {
    var result = await handler.Handle (
      new GetCommentsByAnswerIdsCommand (answerIds), 
      new CancellationToken (false));
  ...
  public async Task<Result<IEnumerable<CommentDTO>>> GetAnswerCommentsAsync (
    ...
    [FromServices] ICommandHandler<IEnumerable<CommentDTO>, GetCommentsCommand> handler) =>
    await handler.Handle (
      new GetCommentsCommand (TypeTarget.Answer, targetId), 
      new CancellationToken (false));
  ...
  public async Task<Result> DeleteAllUserCommentsAsync (
    ...
    await handler.Handle (
      new DeleteAllUserCommentsCommand (userId), 
      new CancellationToken (false));
  ...
  public async Task<Result> DeleteByAnswerIdAsync (
    ...
    await handler.Handle (
      new DeleteByAnswerIdCommand (answerId), 
      new CancellationToken (false));
