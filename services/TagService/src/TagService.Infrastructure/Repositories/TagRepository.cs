...
public class TagRepository : ITagRepository {
  ...
  public async Task<Result<List<Tag>>> GetTagsByIdAsync (List<int> ids, CancellationToken token) {
    List<Tag> tags = await _dbContext.Tags.Where (t => ids.Contains (t.Id)).ToListAsync (token);
  ...
  public async Task<Result<List<Tag>>> GetTagsByNamesAsync (List<string> names, CancellationToken token) {
    List<Tag> tags = await _dbContext.Tags.Where (t => names.Contains (t.Name)).ToListAsync (token);
  ...
  public async Task<Result<(IEnumerable<Tag> items, PagedInfo pageInfo)>> GetTagsAsync (
    ...
    _logger.LogInformation ("GetTagsAsync started. PageParams: {@PageParams}, SortParams: {@SortParams}", pageParams
      , sortParams);

    try {
      var tags = await _dbContext.Tags.Where (TagExtensions.GetSearchFilter (searchFilter)).Sort (sortParams)
       .AsQueryable ().ToPagedAsync (pageParams);
  ...
  /// <summary> Получить список тегов. </summary>
  public async Task<Result<List<Tag>>> GetTagsAsync (IEnumerable<int> ids, CancellationToken token) {
    _logger.LogInformation ("GetTagsAsync started.");
    try {
      var tags = await _dbContext.Tags.Where (tag => ids.Contains (tag.Id)).ToListAsync (token);

      _logger.LogInformation ("GetTagsAsync: получено {Count} тэгов", tags.Count);
      return Result<List<Tag>>.Success (tags);
    }
    catch (Exception ex) {
      _logger.LogError (ex, "GetTagsAsync: ошибка базы данных");
      return Result<List<Tag>>.Error ("Ошибка базы данных");
    }
  }
