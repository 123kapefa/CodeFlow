...
export default function QuestionPage() {
  ...
  const normalizeResponse = (res) => {
    ...
    // обогатим вопрос данными автора
    const qAuthor = usersMap[res.question.userId] ?? {};
    const question = {
      ...res.question,
      authorName: qAuthor.username,
      authorReputation: qAuthor.reputation,
      authorAvatarUrl: qAuthor.avatarUrl,
    };

    // ответы теперь массив; тоже обогащаем авторами
    const answers = (res.answers ?? []).map((a) => {
      const au = usersMap[a.userId] ?? {};
      return {
        ...a,
        authorName: au.username,
        authorReputation: au.reputation,
        authorAvatarUrl: au.avatarUrl,
      };
    });
    ...
    return {
      ...
      tagsMap,
      // комментарии
      questionComments: res.questionComments ?? [],
      answerComments: res.answerComments ?? {},
      // исходные справочники тоже оставим, если понадобятся
      users: res.users ?? [],
      tags: res.tags ?? [],
    };
  };
          ...
          <ul className="list-unstyled mb-2">
            {questionComments.map((c) => (
              <li key={c.id} className="mb-1">
                <span>{c.content}</span>
                <span className="text-muted small ms-2">
                  – {c.name ?? "user"} {dayjs(c.createdAt).fromNow()}
                </span>
              </li>
            ))}
          </ul>
