...
export default function PasswordChangeConfirm() {
  ...
  const emailMasked = useMemo(() => {
    ...
    const safe = name.length <= 2 ? name : name[0] + "*".repeat(name.length - 2) + name.slice(-1);
    return `${safe}@${domain}`;
  }, [email]);
  ...
  if (!email || !token) {
    return (
      <Container className="py-5 text-center">
        <Alert variant="danger">Ссылка некорректна: отсутствует email или token.</Alert>
        ...
        {status !== "ok" && (
          <div className="text-center mt-3">
            <Button variant="primary" onClick={handleConfirm} disabled={busy}>
              {busy ? <Spinner animation="border" size="sm" /> : "Подтвердить смену пароля"}
            </Button>
            <div className="mt-3">
              <Button variant="link" onClick={() => navigate("/")}>Отмена</Button>
            </div>
          </div>
        )}
      </Card>
